...
The original profile used the supplied **“Hi, ESP” development wake model**. The current prototype instead evaluates the required **“Hey Marvin”** phrase locally; see [wake evaluation](hey-marvin-wake.md). Historical Hi ESP trials below do not validate Hey Marvin. The model occupies a new partition at `0x3e0000`; existing ownership, factory and application offsets are preserved. The current runtime feeds the primary microphone plus PCM copied from completed speaker DMA into the AFE (`MR`, full-duplex low-cost configuration). The HAL samples both physical microphones, but this pipeline uses one; the earlier dual-microphone configuration exceeded its processing budget during playback. Muted output produces a zero reference. Shared I2S clocks constrain the initial reference alignment to a DMA window; alignment and attenuation need measurement on the actual assembly. No numerical AEC/wake accuracy claim is made yet.
...
Remaining gates include stable real capture/playback, local pre-roll and wake lifecycle, wake/acoustic corpus, actual audible-stop latency, eight-hour physical soak, full hardware fixtures and measured safety timing. M8 and M9 are not accepted.
...
The isolated `afe-audible` profile enables the amplifier at codec volume50, independently of the computer volume. It preserves the silent profile for unattended work. Short physical speech has traversed the computer speaker, board microphone, live API and board playback driver; see `tests/acceptance/results/M08/attended-paced-audio-smoke.json`. The user confirmed brief speech, then reported crackling and truncated counts. Acoustic quality remains open.
...
After the user confirmed clear count playback, they requested substantially louder output. The attended profile is now configured for codec volume70 (previously50); the silent profile is unchanged. Rebuild/flash and audible verification are recorded separately from the earlier quiet trial.
