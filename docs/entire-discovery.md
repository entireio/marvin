...
| Source bounds / repo state | Repository/CLI docs expose repository workflows; internal API warning applies | M4 requires supported read contracts, revision/source bounds and tenant scopes | The allowlisted local CLI adapter and live `spedemon/marvin` reads pass; hosted multi-user scope still needs a supported service contract |
