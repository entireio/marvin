...
| Bluetooth controller | Firmware includes a Samsung Gear VR ET-YO324 central client and report mapper. The circular touch surface drives forward/reverse and turn like the web drive stick, forward/side IMU tilt controls head pitch/yaw, and the volume buttons adjust speaker volume. Its input shares the same remote-intent loop. | Pairing is not yet tied to the documented deliberate physical gesture, and reconnect/takeover/power-loss/calibration testing is incomplete. Treat it as an experimental controller path. |
