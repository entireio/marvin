...
- `MarvinProfile` has one owner and a designated active conversation. `DeviceBinding` allows zero or one linked device per owner and one owner per device, enforced by transactional storage, not client UI.
...
The active conversation pointer changes only by an explicit conversation choice/new conversation, not merely opening an old transcript. A body resolves the designated conversation at interaction start. Repository changes serialize with active generation and rebuild context; repository caches are keyed by owner/connection/repository/revision and invalidated on authorization changes.
