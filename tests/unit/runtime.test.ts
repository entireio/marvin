...
describe('capability policy',()=>{
 ...
 it('body motion guidance calls first and answers in the pet’s voice',()=>{expect(persona).toContain('call it immediately before producing any text');expect(persona).toContain('say exactly one very short, first-person acknowledgement');expect(persona).toContain('speaking as the Desktop Pet itself');expect(persona).toContain('never say that you will send a head-turn command');expect(persona).toContain('Do not add a second sentence, status narration, uncertainty, or a completion claim.');});
});
it('the deterministic preview executes direct body motion and stays brief',async()=>{
 ...
 expect(calls).toEqual([{name:'physical_head',args:{yaw:40,pitch:0,durationMs:550}}]);expect(events).toEqual([{type:'delta',text:'Sure.'}]);
 calls.length=0;for await(const _ of provider.run({...context,input:'Look down.'},async(name,args)=>{calls.push({name,args});return {state:'accepted'};},new AbortController().signal)){};expect(calls).toEqual([{name:'physical_head',args:{yaw:0,pitch:-30,durationMs:550}}]);
 ...
 calls.length=0;for await(const _ of provider.run({...context,input:'Hey Marvin, walk forward.'},async(name,args)=>{calls.push({name,args});return {state:'accepted'};},new AbortController().signal)){};expect(calls).toEqual([{name:'physical_motion',args:{primitive:'forward_bit'}}]);
 calls.length=0;for await(const _ of provider.run({...context,input:'Hey Marvin, move backward.'},async(name,args)=>{calls.push({name,args});return {state:'accepted'};},new AbortController().signal)){};expect(calls).toEqual([{name:'physical_motion',args:{primitive:'backward_bit'}}]);
 calls.length=0;for await(const _ of provider.run({...context,input:'Hey Marvin, turn around.'},async(name,args)=>{calls.push({name,args});return {state:'accepted'};},new AbortController().signal)){};expect(calls).toEqual([{name:'physical_motion',args:{primitive:'turn_around'}}]);
});
