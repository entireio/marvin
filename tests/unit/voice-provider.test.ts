...
for(const surface of ['web_voice','body_voice'] as const)it(`real SDK configures PCM/VAD and scopes reconstructed context/tools to ${surface}`,async()=>{
 ...
 const first=requests.find(e=>e.type==='response.create').response;expect(first.conversation).toBe('none');expect(first.input.at(-1)).toEqual({type:'item_reference',id:'audio-item'});expect(JSON.stringify(first.input)).toContain('blue light');expect(JSON.stringify(first.input)).toContain('calibration value 42');expect(first.instructions).toContain('Surface: '+surface+'.');expect(first.instructions).toContain('active repository display name is "spidemon / miraloma_robotics"');expect(first.instructions).not.toContain('Active repository: marvin-firmware');expect(first.tools.map((t:any)=>t.name)).toEqual(surface==='body_voice'?['repository_summary','physical_head']:['repository_summary']);expect(requests.filter(e=>e.type==='response.create')[1].response.input.at(-1).type).toBe('function_call_output');
