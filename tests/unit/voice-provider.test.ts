...
const context:ModelContext={interaction:{ownerId:'test',conversationId:'thread',interactionId:'turn',routeId:'voice-route',surface:'web_voice',repositoryId:'marvin-firmware',repositoryName:'spidemon / miraloma_robotics',entireState:'fixture',body:{deviceId:'robot',status:'online',capabilities:['head']}},summary:'Earlier: calibration value 42.',messages:[{userText:'Remember the blue light',assistantText:'I will remember.',status:'completed'}],input:'What changed?'};
for(const surface of ['web_voice','body_voice'] as const)it(`real SDK configures PCM/VAD and scopes reconstructed context/tools to ${surface}`,async()=>{
 ...
 const provider=new OpenAIVoiceProvider('fixture-key','test-model','test-transcribe','cedar',`https://127.0.0.1:${port}/v1`,ca);const connection=await provider.connect(()=>{});
 ...
 expect(calls).toEqual(['repository_summary']);expect(events.some(e=>e.type==='audio')).toBe(true);expect(events.some(e=>e.type==='delta'&&e.text==='Grounded spoken answer.')).toBe(true);expect(events.some(e=>e.type==='card')).toBe(true);
  const setup=requests.find(e=>e.type==='session.update').session;expect(setup.audio.input.turn_detection).toMatchObject({create_response:false,interrupt_response:false});expect(setup.audio.input.format).toEqual({type:'audio/pcm',rate:24000});expect(setup.audio.output.voice).toBe('cedar');expect(setup.instructions).toContain('neutral-to-masculine');
  const first=requests.find(e=>e.type==='response.create').response;expect(first.conversation).toBe('none');expect(first.input.at(-1)).toEqual({type:'item_reference',id:'audio-item'});expect(JSON.stringify(first.input)).toContain('blue light');expect(JSON.stringify(first.input)).toContain('calibration value 42');expect(first.instructions).toContain('Surface: '+surface+'.');expect(first.instructions).toContain('emotionally contained');expect(first.instructions).toContain('active repository display name is "spidemon / miraloma_robotics"');expect(first.instructions).not.toContain('Active repository: marvin-firmware');expect(first.tools.map((t:any)=>t.name)).toEqual(surface==='body_voice'?['repository_summary','physical_head']:['repository_summary']);expect(requests.filter(e=>e.type==='response.create')[1].response.input.at(-1).type).toBe('function_call_output');
...
it('synthesizes a bounded lifecycle announcement without creating a conversation turn',async()=>{const {ws:server,http}=await tlsServer();const requests:any[]=[];server.on('connection',ws=>ws.on('message',raw=>{const e=JSON.parse(raw.toString());requests.push(e);const send=(event:object)=>ws.send(JSON.stringify({event_id:'fixture',...event}));if(e.type==='session.update')send({type:'session.updated',session:{type:'realtime'}});if(e.type==='response.create'){const response={id:'announcement',metadata:e.response.metadata,status:'completed',output:[]};send({type:'response.created',response});send({type:'response.output_audio.delta',response_id:response.id,delta:Buffer.alloc(960).toString('base64')});send({type:'response.done',response});}}));const connection=await new OpenAIVoiceProvider('fixture','test','test','marin',`https://127.0.0.1:${(http.address() as {port:number}).port}/v1`,ca).connect(()=>{});try{const audio=[];for await(const pcm of connection.speak!('Account linked.',AbortSignal.timeout(3000)))audio.push(pcm);expect(audio.reduce((n,b)=>n+b.length,0)).toBe(960);const request=requests.find(e=>e.type==='response.create').response;expect(request.metadata.marvin_lifecycle).toBe('linked');expect(request.input[0].content[0].text).toBe('Account linked.');}finally{connection.close();for(const ws of server.clients)ws.terminate();await new Promise<void>(r=>server.close(()=>http.close(()=>r())));}});
