...
for(const mode of ['sqlite',...(process.env.TEST_DATABASE_URL?['postgres']:[])])describe(`enrollment conformance: ${mode}`,()=>{
 ...
 function request(operation:'claim'|'network'='claim'){const challenge={deviceId:device,nonce:randomBytes(32).toString('hex'),operation,issuedAt:Date.now()};return {challenge,proof:proof(challengeMessage(challenge))};}
