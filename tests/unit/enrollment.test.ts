...
for(const mode of ['sqlite',...(process.env.TEST_DATABASE_URL?['postgres']:[])])describe(`enrollment conformance: ${mode}`,()=>{
 ...
 it('authorizes the former owner to reconcile the exact revoked epoch once',async()=>{const {ticket}=await service.ticket(owner,request());await redeem(ticket);await store.unlink(owner);const reconciliation=await service.ticket(owner,request('reconcile'));const claims=JSON.parse(Buffer.from(reconciliation.ticket.split('.')[1],'base64url').toString());expect(claims).toMatchObject({aud:device,sub:owner,op:'reconcile',epoch:1});await expect(service.ticket(other,request('reconcile'))).rejects.toMatchObject({code:'RECOVERY_NOT_AUTHORIZED'});expect(await store.acknowledgeRevocation(owner,device)).toBe(true);await expect(service.ticket(owner,request('reconcile'))).rejects.toMatchObject({code:'RECOVERY_NOT_AUTHORIZED'});});
 it('recovers a pre-migration unlink from its completed claim receipt',async()=>{const {ticket}=await service.ticket(owner,request());await redeem(ticket);await store.unlink(owner);await db.query('DELETE FROM device_revocations WHERE device_id=?',[device]);await expect(service.ticket(owner,request('reconcile'))).resolves.toMatchObject({enrollmentId:expect.any(String)});});
