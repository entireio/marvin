...
it('Pet conversation link can be selected independently and rejects foreign conversations',async()=>{const first=await service.store.createConversation(ownerId),second=await service.store.createConversation(ownerId),other=await service.store.ensureOwner('test',randomUUID(),'Other'),foreign=await service.store.createConversation(other.id);expect((await service.app.inject({url:'/api/robot/conversation',headers:headers()})).json()).toEqual({conversationId:null});expect((await service.app.inject({method:'PUT',url:'/api/robot/conversation',headers:headers(),payload:{conversationId:first.id}})).json()).toEqual({conversationId:first.id});await service.store.activate(ownerId,second.id);expect((await service.app.inject({url:'/api/robot/conversation',headers:headers()})).json()).toEqual({conversationId:first.id});expect((await service.app.inject({method:'PUT',url:'/api/robot/conversation',headers:headers(),payload:{conversationId:foreign.id}})).statusCode).toBe(404);expect((await service.app.inject({method:'PUT',url:'/api/robot/conversation',headers:headers(),payload:{conversationId:null}})).json()).toEqual({conversationId:null});});
...
it('websocket output is origin-routed, replayable, and stops immediately after logout',async()=>{
 ...
 await service.app.inject({method:'PUT',url:'/api/robot/conversation',headers:headers(),payload:{conversationId:c.id}});
 await new Promise(r=>setTimeout(r,20));expect(observed.every(events=>events.some(e=>e.type==='conversation_link'))).toBe(true);
 ...
 expect(observed[0].some(e=>e.type==='completed')).toBe(true);expect(observed[1].some(e=>e.type==='delta')).toBe(true);
