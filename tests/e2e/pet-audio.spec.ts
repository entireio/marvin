...
test('connected Pet audio controls stay synchronized between the header and Settings',async({page})=>{
 let audio={volume:60,muted:false,microphoneGainDb:30};
 ...
 await page.getByRole('button',{name:'Mute Desktop Pet microphone'}).first().click();await expect.poll(()=>audio).toEqual({volume:35,muted:true,microphoneGainDb:30});
 ...
 await expect(page.getByRole('banner').getByLabel('Desktop Pet microphone sensitivity')).toHaveCount(0);await dialog.getByLabel('Desktop Pet microphone sensitivity').fill('12');await expect.poll(()=>audio).toEqual({volume:35,muted:true,microphoneGainDb:12});
