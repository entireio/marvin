...
test('header and remote control show live battery level and charging status',async({page})=>{
 ...
 await expect(remote.getByLabel('Desktop Pet battery 74%')).toBeVisible();
 battery={...battery,charging:true};
 await expect(remote.getByLabel('Desktop Pet battery 74%, charging')).toBeVisible({timeout:3000});
 battery={levelPercent:19,voltageMv:3735,charging:false};
 await expect(remote.getByLabel('Desktop Pet battery 19%')).toBeVisible({timeout:3000});
});
