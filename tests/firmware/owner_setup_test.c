...
static void ok(uint32_t session,const char *json,const char *field){cJSON *r=command(session,json);assert(cJSON_GetObjectItem(r,field)&&!cJSON_GetObjectItem(r,"error"));cJSON_Delete(r);}
...
int main(int argc,char **argv){assert(argc==3);public_key=file(argv[1]);char *raw=file(argv[2]);cJSON *cases=cJSON_Parse(raw);const char *ticket=cJSON_GetObjectItem(cJSON_GetArrayItem(cases,0),"ticket")->valuestring;assert(marvin_owner_setup_init()==ESP_OK);
 ...
 cJSON_Delete(cases);free(raw);free(public_key);puts("Owner setup: real ES256 ticket transfer, session fence, idempotent finish, persistence failure, reboot/redeem retry and challenge expiry passed.");
}
