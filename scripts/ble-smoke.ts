...
try{
 ...
 let chosen=-1;const networks=[];for(let i=0;i<status.count;i++){const n=await command({op:'network',index:i});if(n.source!=='robot'||n.channel<1||n.channel>14)throw new Error('Invalid robot radio result');if(wifi&&chosen<0&&n.ssid===wifi.ssid&&n.supported)chosen=i;networks.push({channel:n.channel,rssi:n.rssi,supported:n.supported});}
 if(wifi){
 ...
 mkdirSync('work/board',{recursive:true});const result={at:new Date().toISOString(),passed:true,security:2,patch:1,deviceProofVerified:true,networkCount:networks.length,networks,note:wifi?'SSID and setup secrets excluded; Wi-Fi result recorded separately.':'SSID and setup secrets excluded. Scan only; no Wi-Fi configuration or ownership claim.'};writeFileSync('work/board/ble-smoke.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
}
