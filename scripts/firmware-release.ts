...
const options=z.object({key:z.string().min(1),image:z.string().min(1),sequence:z.coerce.number().int().min(1).max(4294967295),'minimum-sequence':z.coerce.number().int().min(0).max(4294967295),layout:z.enum(['owner-v1','afe-v1','afe-v2']),out:z.string().min(1)}).parse(values);
if(options['minimum-sequence']>=options.sequence)throw new Error('Minimum prior sequence must precede this release.');
const binary=readFileSync(options.image);const slotBytes=options.layout==='afe-v2'?0x300000:0x1e0000;if(binary.length<1024||binary.length>slotBytes||binary[0]!==0xe9||binary.readUInt16LE(12)!==9)throw new Error('Use an ESP32-S3 application image that fits the reviewed OTA slot.');
