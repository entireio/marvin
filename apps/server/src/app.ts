...
import { SESSION_LIFETIME_MS,SESSION_RENEWAL_WINDOW_MS,Store } from '../../../packages/persistence/src/store.js';
...
import { EntireCloudRouter } from '../../../packages/runtime/src/entire-router.js';
import { EntireConnectorBroker } from './entire-connector.js';
...
export async function createApp(cfg:Config,options:{database?:Database;provider?:TextProvider;logger?:boolean;entire?:RepositoryIntegration;voice?:VoiceProvider;voiceLimits?:{idleMs:number;maxMs:number;heartbeatMs:number}}={}){
 ...
 const hostedEntire=cfg.ENTIRE_HOSTED_CLI_PATH&&cfg.ENTIRE_HOSTED_SECRET_DIR&&cfg.ENTIRE_HOSTED_SECRET_KEY?new EntireHostedCli(cfg.ENTIRE_HOSTED_CLI_PATH,new EntireSecretStore(cfg.ENTIRE_HOSTED_SECRET_DIR,cfg.ENTIRE_HOSTED_SECRET_KEY),store):undefined;
 const connectorEntire=cfg.ENTIRE_CONNECTOR_ENABLED==='true'?new EntireConnectorBroker(store):undefined;
 const cloudEntire=hostedEntire||connectorEntire?new EntireCloudRouter(store,hostedEntire,connectorEntire):undefined;
 const realEntire=options.entire??(cfg.ENTIRE_CLI_PATH&&cfg.ENTIRE_BINDINGS_FILE?new EntireCli(cfg.ENTIRE_CLI_PATH,cfg.ENTIRE_BINDINGS_FILE):cfg.ENTIRE_COMPANION_URL&&cfg.ENTIRE_COMPANION_TOKEN?new EntireHttp(cfg.ENTIRE_COMPANION_URL,cfg.ENTIRE_COMPANION_TOKEN):cloudEntire);
 ...
 const localDevPets=cfg.LOCAL_DEV_PETS_FILE?(()=>{
  const card=z.object({deviceId:z.string().regex(/^marvin_[a-f0-9]{32}$/),username:z.string().min(1).max(64),password:z.string().min(8).max(128),security:z.literal(2).optional(),patch:z.literal(1).optional()}).strict(),entry=z.object({label:z.string().min(1).max(64),cardFile:z.string().startsWith('/'),devicePublicKeyFile:z.string().startsWith('/')}).strict(),entries=z.array(entry).min(1).max(64).parse(JSON.parse(readFileSync(cfg.LOCAL_DEV_PETS_FILE,'utf8'))),pets=entries.map(item=>({...item,card:card.parse(JSON.parse(readFileSync(item.cardFile,'utf8')))}));
 ...
 const cookieOptions={path:'/',httpOnly:true,sameSite:'lax' as const,secure,maxAge:SESSION_LIFETIME_MS/1000};
 const activeSession=async(req:{cookies:Record<string,string|undefined>},reply?:FastifyReply)=>{
  const token=req.cookies.marvin_session,s=await store.session(token);
  if(s&&token&&s.expiresAt-Date.now()<=SESSION_RENEWAL_WINDOW_MS&&await store.renewSession(token))reply?.setCookie('marvin_session',token,cookieOptions);
  return s;
 };
 ...
 app.addHook('onRequest',async(req,reply)=>{
  ...
  if(req.url.startsWith('/api/auth/')||req.url.split('?')[0]==='/api/config'||req.url==='/api/health'||req.url==='/api/device/socket'||req.url.split('?')[0]==='/api/entire/connector/socket')return;
  const session=await activeSession(req,reply);if(!session)throw new DomainError('UNAUTHENTICATED','Please sign in to continue.',401);
 ...
 app.get('/api/health',async()=>({status:'ok',schema:10}));
 ...
 app.get('/api/auth/session',async(req,reply)=>{const s=await activeSession(req,reply);return s?{owner:await store.owner(s.ownerId),csrf:s.csrf}:null;});
 ...
 app.get('/api/robot/presence',async(req)=>({body:await store.body((await session(req)).ownerId),presence:devices.publicPresence((await session(req)).ownerId)}));
 ...
 app.get('/api/settings',async(req)=>{const s=await session(req),o=await store.owner(s.ownerId),entireConnection=await store.entireConnection(s.ownerId);let available=false,repositories=o.entireState==='fixture'?fixtureRepositories:[],entireError:string|undefined;
  ...
  const entireStatus=entireConnection?.status==='reauth_required'?'reauth_required':entireError?'configuration_error':o.entireState==='connected'?'connected':!realEntire?'host_unavailable':available?'ready':hostedEntire||connectorEntire?'connection_required':'repository_required';
  return {entireState:o.entireState,entireStatus,entireAvailable:available,entireError,entireConnection:entireConnection?{method:entireConnection.authKind,status:entireConnection.status,cliVersion:entireConnection.cliVersion,lastVerifiedAt:entireConnection.lastVerifiedAt}:null,entireMethods:{hosted:!!hostedEntire,connector:!!connectorEntire,connectorOnline:connectorEntire?.online(s.ownerId)??false},body:await store.body(s.ownerId),repositories,provider:runtime.provider.name};});
 app.post('/api/entire/login',{config:{rateLimit:{max:5,timeWindow:'1 minute'}}},async(req)=>{z.object({}).strict().parse(req.body);if(!hostedEntire)throw new DomainError('ENTIRE_HOSTED_UNAVAILABLE','Hosted Entire sign-in is not configured.',409);const ownerId=(await session(req)).ownerId,existing=await store.entireConnection(ownerId);if(existing?.authKind==='connector'){connectorEntire?.disconnect(ownerId);await store.revokeEntireConnector(ownerId);await store.removeEntireConnection(ownerId);}return hostedEntire.startLogin(ownerId);});
 app.get('/api/entire/login/:id',async(req)=>{if(!hostedEntire)throw new DomainError('ENTIRE_HOSTED_UNAVAILABLE','Hosted Entire sign-in is not configured.',409);const {id}=z.object({id:Id}).parse(req.params);return hostedEntire.loginStatus((await session(req)).ownerId,id);});
 app.delete('/api/entire/login/:id',async(req)=>{if(!hostedEntire)throw new DomainError('ENTIRE_HOSTED_UNAVAILABLE','Hosted Entire sign-in is not configured.',409);const {id}=z.object({id:Id}).parse(req.params);return {cancelled:hostedEntire.cancelLogin((await session(req)).ownerId,id)};});
 app.post('/api/entire/connector/pairing',{config:{rateLimit:{max:5,timeWindow:'1 minute'}}},async(req)=>{z.object({}).strict().parse(req.body);if(!connectorEntire)throw new DomainError('ENTIRE_CONNECTOR_UNAVAILABLE','The Entire connector fallback is not enabled.',409);const ownerId=(await session(req)).ownerId,existing=await store.entireConnection(ownerId);if(existing?.authKind==='hosted_cli')await hostedEntire?.disconnect(ownerId);const pairing=await store.createEntireConnectorPairing(ownerId),serverUrl=cfg.APP_ORIGIN,socketUrl=cfg.APP_ORIGIN.replace(/^http/,'ws')+'/api/entire/connector/socket';return {...pairing,serverUrl,socketUrl};});
 if(connectorEntire)app.get('/api/entire/connector/socket',{websocket:true},socket=>connectorEntire.attach(socket));
 ...
 app.addHook('onClose',async()=>{clearInterval(recover);clearInterval(retention);voice.closeAll();hostedEntire?.close();connectorEntire?.closeAll();await devices.voice?.close();devices.close();await runtime.close();await retentionWork;await db.close();});
