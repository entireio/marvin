...
esp_err_t marvin_identity_challenge(const char *operation,double browser_time,cJSON *reply){
 ...
 if((strcmp(operation,"claim")&&strcmp(operation,"network"))||!isfinite(browser_time)||browser_time<1||browser_time>9007199254740991.0||floor(browser_time)!=browser_time)return ESP_ERR_INVALID_ARG;
