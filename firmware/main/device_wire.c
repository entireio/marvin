...
bool marvin_wire_control(const char *text,uint32_t epoch,bool welcomed){
 ...
 if(unique(m)&&cJSON_IsString(type)&&!strcmp(type->valuestring,"welcome")&&!welcomed){
  ...
  ok=unique(p)&&number(cJSON_GetObjectItemCaseSensitive(p,"major"),1)&&number(cJSON_GetObjectItemCaseSensitive(p,"minor"),2)&&number(cJSON_GetObjectItemCaseSensitive(m,"epoch"),epoch)&&number(cJSON_GetObjectItemCaseSensitive(m,"heartbeatMs"),5000);
