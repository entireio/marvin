...
/* Marvin patch: pinned ESP-IDF5.4.2 transport accessor, also used by IDF's
 * transport tests. Review this private boundary on an IDF upgrade. */
int esp_transport_get_socket(esp_transport_handle_t transport);
...
esp_websocket_client_handle_t esp_websocket_client_init(const esp_websocket_client_config_t *config)
{
    ...
    client->config->tcp_nodelay = config->tcp_nodelay;
...
static void esp_websocket_client_task(void *pv)
{
    ...
    while (client->run) {
        ...
        case WEBSOCKET_STATE_INIT:
            ...
            if (client->config->tcp_nodelay) {
                int fd = esp_transport_get_socket(client->transport), enabled = 1;
                if (fd < 0 || setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &enabled, sizeof(enabled)) != 0) {
                    esp_websocket_client_error(client, "Realtime TCP option unavailable");
                    esp_websocket_client_abort_connection(client, WEBSOCKET_ERROR_TYPE_TCP_TRANSPORT);
                    break;
                }
            }
