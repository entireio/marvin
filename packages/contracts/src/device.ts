...
export const DeviceHello=z.object({type:z.literal('hello'),protocol:z.object({major:z.number().int(),minor:z.number().int()}).strict(),deviceId:Id,bootId:Id,capabilities:z.array(DeviceCapability).max(10),firmware:z.string().max(64),audioInputRate:z.literal(16000).optional(),audioSettings:PetAudioSettings.optional(),headCalibration:HeadCalibration.optional(),batteryStatus:BatteryStatus.optional()}).strict();
