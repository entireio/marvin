...
export const PetAudioSettings=z.object({volume:z.number().int().min(0).max(100),muted:z.boolean()}).strict();
...
export const DeviceControl=z.discriminatedUnion('type',[
 ...
 z.object({type:z.literal('audio_settings'),volume:z.number().int().min(0).max(100),muted:z.boolean()}).strict()
]);
