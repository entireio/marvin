...
import {persona,priorUserText,repositoryToolRounds,type TextProvider,type ModelContext,type ToolExecutor,type ModelEvent} from './provider.js';
...
export class AnthropicTextProvider implements TextProvider {
 ...
 async *run(ctx:ModelContext,execute:ToolExecutor,signal:AbortSignal):AsyncIterable<ModelEvent>{
  ...
  for(let round=0;round<=repositoryToolRounds;round++){
   const stream=this.client.messages.stream({model:this.model,max_tokens:2400,system:`${persona}\nSurface: ${ctx.interaction.surface}. ${bodyExplanation(ctx.interaction)}\nActive repository: ${ctx.interaction.repositoryId??'none'}. Entire state: ${ctx.interaction.entireState}.`,messages,tools:allowedTools(ctx.interaction).map(t=>({name:t.name,description:t.description,input_schema:t.parameters as Anthropic.Tool.InputSchema})),...(round===repositoryToolRounds?{tool_choice:{type:'none' as const}}:{})},{signal});
   let bytes=0;
   try{for await(const event of stream){signal.throwIfAborted();bytes+=JSON.stringify(event).length;if(bytes>512000)throw new Error('Provider response exceeded its limit.');if(event.type==='content_block_delta'&&event.delta.type==='text_delta')yield {type:'delta',text:event.delta.text};}
    ...
    for(const call of calls){signal.throwIfAborted();yield {type:'tool',text:call.name.startsWith('physical_')?'Sending a request to Marvin…':'Reading repository context…'};let result:unknown,isError=false;
