...
export class EntireCli implements RepositoryIntegration {
 ...
 async configured(ownerId:string):Promise<Repository[]>{const b=await this.binding(ownerId);return b.repositories.map(r=>({id:repositoryId(r.name),name:r.name,description:r.checkout?'Entire + committed local source':'Entire repository',source:'entire',capabilities:['summary','search','code_search',...(r.checkout?['source','history','checkpoint','session']:[]),...(r.checkout&&r.graph?['graph']:[])]}));}
 async check(ownerId:string,id:string,signal?:AbortSignal){const b=await this.binding(ownerId),repo=b.repositories.find(r=>repositoryId(r.name)===id);if(!repo)throw new DomainError('REPOSITORY_FORBIDDEN','This repository is not authorized for your account.',403);
 ...
 async discover(ownerId:string,cursor?:string){const b=await this.binding(ownerId);const args=['repo','mirror','list','--mirrored','--json','--page-size','25'];if(cursor)args.push('--page-token',z.string().max(2048).parse(cursor));const page=validate(z.object({items:z.array(z.object({repo:z.string(),status:z.string()})).max(500),nextPageToken:z.string().optional()}),parse(await this.cli(b,args)));const allowed=new Set(b.repositories.map(r=>r.name));return {items:page.items.filter(r=>allowed.has(remoteName(r.repo))).map(r=>({id:repositoryId(remoteName(r.repo)),name:remoteName(r.repo),description:r.status,source:'entire' as const,capabilities:[]})),nextCursor:page.nextPageToken||null};}
