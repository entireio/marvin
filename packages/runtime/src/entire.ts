...
export class FixtureEntire implements EntireAdapter {
 ...
 async readSummary(id:string):Promise<RepoCard>{
  ...
  return {kind:'repository',title:id==='marvin-firmware'?'A clearer path from setup to conversation':'The personal Marvin portal',repositoryId:id,revision:'fixture-001',source:'fixture',summary:'Illustrative repository data for development. The current slice separates Wi-Fi provisioning, conversation state, and interaction-scoped tools.',files:[{path:'provisioning/network.ts',description:'Marvin scans for compatible 2.4 GHz networks; failed changes keep the previous configuration.'},{path:'runtime/policy.ts',description:'Physical actions are available only through the embodied Marvin.'},{path:'conversation/store.ts',description:'Conversation state remains in Marvin across model sessions.'}]};
 }
}
