...
export class DeviceStore {
 ...
 async create(identity:DeviceIdentity,command:DeviceCommand){return this.store.db.transaction(async tx=>{
  ...
  const n=(await tx.query<{n:number}>("SELECT COUNT(*) AS n FROM device_commands WHERE device_id=? AND state IN ('sent','accepted') AND deadline>?",[identity.deviceId,Date.now()]))[0].n;if(Number(n)>=16)throw new DomainError('DEVICE_BUSY','Your Desktop Pet has too many pending actions.',429);
 ...
 async greetingPending(identity:DeviceIdentity){return (await this.store.db.query("SELECT device_id FROM body_slots WHERE device_id=? AND owner_id=? AND epoch=? AND state='linked' AND simulated=0 AND greeted_at IS NULL",[identity.deviceId,identity.ownerId,identity.epoch])).length>0;}
 async markGreeted(identity:DeviceIdentity){return (await this.store.db.query("UPDATE body_slots SET greeted_at=? WHERE device_id=? AND owner_id=? AND epoch=? AND state='linked' AND greeted_at IS NULL RETURNING device_id",[Date.now(),identity.deviceId,identity.ownerId,identity.epoch])).length>0;}
