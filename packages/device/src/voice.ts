...
type Session={identity:DeviceIdentity;connection?:VoiceConnection;sink:DeviceVoiceSink;closed:boolean;generation:number;activeId:string;lastId:string;conversationId:string;lastActivity:number;started:number;playhead:number;seen:Set<string>;queue:Promise<void>;resampler?:Pcm16To24};
/** Provider sessions exist only between an explicit wake/button and stop/idle timeout. */
export class DeviceVoice {
 private sessions=new Map<string,Session>();private timer:ReturnType<typeof setInterval>;
 constructor(readonly runtime:Runtime,readonly provider?:VoiceProvider){this.timer=setInterval(()=>{for(const s of this.sessions.values())if(Date.now()-s.lastActivity>90000||Date.now()-s.started>900000)void this.stop(s.identity.deviceId);},1000);}
 async start(identity:DeviceIdentity,sink:DeviceVoiceSink,inputSampleRate:16000|24000=24000){
  ...
  const s:Session={identity,sink,closed:false,generation:0,activeId:'',lastId:'',conversationId:'',lastActivity:Date.now(),started:Date.now(),playhead:0,seen:new Set(),queue:Promise.resolve(),...(inputSampleRate===16000?{resampler:new Pcm16To24()}:{})};this.sessions.set(identity.deviceId,s);
 ...
 private signal(s:Session,e:VoiceSignal){if(!this.current(s))return;s.lastActivity=Date.now();
  ...
  s.queue=s.queue.then(async()=>{const stale=()=>!this.current(s)||s.generation!==generation;if(stale()||!s.connection)return;await this.flush(s);if(stale())return;const text=e.text.trim();if(!text)return;if(text.length>12000)throw new Error('Spoken turn too long');
   ...
   await this.runtime.waitTurn(id);if(!s.closed&&s.activeId===id){s.activeId='';s.playhead=0;await s.sink.control({type:'voice_turn_end',interactionId:id});}
