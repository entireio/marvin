...
function response(data:Uint8Array,msg:number,id:number){const outer=parse(data);if(outer.get(2)!==2)throw new Error('Desktop Pet must use Security 2.');const inner=parse(bytes(outer,12));if(inner.get(1)!==msg)throw new Error('Unexpected setup response.');const result=parse(bytes(inner,id));if((result.get(1)??0)!==0)throw new Error('Setup code was rejected.');return result;}
...
export class Security2 {
 ...
 async open(username:string,password:string,exchange:(data:Uint8Array)=>Promise<Uint8Array>){
  ...
  try{const A=bin(pow(G,num(secret)));const first=response(await exchange(packet(0,20,cat(field(1,utf8.encode(username)),field(2,A)))),1,21);const derived=await deriveSrp(username,password,secret,bytes(first,3),bytes(first,2));derivedKey=derived.key;if(this.failed)throw new Error('Secure setup was cancelled.');const last=response(await exchange(packet(2,22,field(1,derived.proof))),3,23),proof=bytes(last,2),nonce=bytes(last,3);
   let mismatch=proof.length^derived.serverProof.length;for(let i=0;i<64;i++)mismatch|=(proof[i]??0)^derived.serverProof[i];if(mismatch||nonce.length!==12)throw new Error('Desktop Pet did not prove the setup secret.');
