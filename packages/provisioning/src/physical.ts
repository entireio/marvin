...
export class PhysicalRobot implements ProvisioningTransport {
 ...
 private async command(request:Record<string,unknown>){const response=Reply.parse(await this.raw(request,'Reconnect to Marvin to continue.'));if(this.closed)throw new DomainError('BLUETOOTH_DISCONNECTED','Reconnect to check setup progress.');if(response.error)throw new DomainError(response.error,this.explanation(response.error));return response;}
 ...
 private explanation(code:string){return ({WIFI_CONNECTION_FAILED:'Marvin could not join that network. Check the password and signal, then try again.',WIFI_AUTH_FAILED:'That Wi-Fi password did not work. Check it and try again.',PREVIOUS_NETWORK_UNAVAILABLE:'The new network failed and Marvin could not return to its previous network. Open setup again to choose another network.',OWNER_TICKET_REQUIRED:'Setup authorization expired. Start the connection again.',SETUP_PENDING:'Marvin is still confirming setup. Open setup again to check its progress.',ENROLLMENT_RETRY:'Marvin joined Wi-Fi but could not confirm the account link. Open setup again when the service is available.',BACKEND_UNREACHABLE:'Marvin joined Wi-Fi but could not contact the Marvin service.',TLS_OR_TRANSPORT_FAILED:'Marvin joined Wi-Fi but could not establish a secure connection to the Marvin service.',BACKEND_HTTP_FAILED:'The Marvin service did not accept the connectivity check.',CLOCK_SYNC_FAILED:'Marvin joined Wi-Fi but could not set its clock securely.',ENROLLMENT_REVOKED:'This setup is no longer authorized. Start setup again.',CHALLENGE_EXPIRED:'Setup took too long. Start the connection again.',OWNERSHIP_STATE:'Marvin’s ownership state changed. Close setup and try again.',RECOVERY_NOT_AUTHORIZED:'This Marvin still contains another account’s link. Ask its previous owner to unlink it, or use the physical recovery process.',TICKET_INVALID:'Marvin could not verify the account authorization. Close setup and try again.',INVALID_REQUEST:'Marvin and this application use incompatible setup versions.',BACKEND_NOT_CONFIGURED:'This Marvin does not have a setup service configured.'} as Record<string,string>)[code]??'Marvin could not complete this setup step. Close setup and try again.';}
 ...
 private async authorization(operation:'claim'|'network'|'reconcile',signal:AbortSignal){
  ...
  try{signal.throwIfAborted();if(reply.expiresAt<=Date.now())throw new DomainError('SETUP_EXPIRED','Setup expired. Try again.');
   ...
   const verified=await this.command({op:'ticket_finish'});if(verified.verified!==true)throw new DomainError('TICKET_REJECTED','Marvin did not verify account authorization.');return reply.enrollmentId;
 ...
 async reconcile(progress:(stage:ProvisioningStage)=>void,signal:AbortSignal){
  ...
  if(binding){if(!binding.simulated&&binding.device_id===this.deviceId)return false;throw new DomainError('ACTIVE_BINDING','This Marvin is linked to a different account or device record.');}
  if(!this.link.reconciliation)throw new DomainError('FIRMWARE_RECONCILIATION_REQUIRED','Marvin was removed from your account, but this firmware cannot clear its previous link automatically. Update or physically reset Marvin, then start setup again.');
  progress('reconciling');let enrollmentId:string|undefined;
  try{enrollmentId=await this.authorization('reconcile',signal);const cleared=await this.command({op:'clear_owner'});if(cleared.cleared!==true)throw new DomainError('OWNERSHIP_STATE','Marvin could not clear its previous account link.');await this.server.acknowledge(this.deviceId);return true;}
 ...
 async scan():Promise<ScanResult>{
  ...
  if(status.phase!=='scan_complete')throw new DomainError('SCAN_TIMEOUT','Marvin took too long to scan Wi-Fi. Try again.');this.generation=status.scanGeneration;const scannedAt=Date.now(),networks:Network[]=[],byIdentity=new Map<string,Network>();
 ...
 async connect(network:Network,password:string,progress:(stage:ProvisioningStage)=>void,signal:AbortSignal){
  const chosen=this.selected.get(network.id),index=this.indices.get(network.id),hidden=this.hidden.has(network.id);if(!chosen||(!hidden&&index===undefined)||chosen.ssid!==network.ssid||!chosen.compatible)throw new DomainError('NETWORK_NOT_SCANNED','Choose a supported network from Marvin’s scan.');
 ...
 private async waitForReady(network:string,progress:(stage:ProvisioningStage)=>void,signal:AbortSignal){
  ...
  do{signal.throwIfAborted();const status=await this.status();signal.throwIfAborted();
   ...
   if((status.phase==='network_verified'||(status.phase==='idle'&&!status.pendingNetwork&&status.savedNetwork===network))&&status.linked&&status.networkConnected&&status.hasSavedNetwork){const body=await this.server.binding();signal.throwIfAborted();if(body&&!body.simulated&&body.device_id===this.deviceId&&body.network===network){progress('ready');return;}throw new DomainError('STALE_LOCAL_LINK','Marvin still contains an account link that was removed from the server. Close setup and connect again so it can be cleared safely.');}await this.pause(signal);
