...
export function decodeSetupCode(value:string){try{const text=value.trim();if(!/^marvin1\.[A-Za-z0-9_-]{1,512}$/.test(text))throw new Error();const raw=text.slice(8).replace(/-/g,'+').replace(/_/g,'/');return Card.parse(JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(Uint8Array.from(atob(raw),c=>c.charCodeAt(0)))));}catch{throw new DomainError('SETUP_CODE_INVALID','Use the complete setup code from your Desktop Pet’s setup card.');}}
