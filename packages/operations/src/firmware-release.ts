...
export class FirmwareRelease {
 ...
 constructor(configFile:string){
  ...
  const manifest=boundedFile(join(options.bundleDirectory,'manifest.bin'),176),image=boundedFile(join(options.bundleDirectory,'image.bin'),0x400000);
  ...
  const slotBytes=this.layout==='afe-v3'?0x400000:this.layout==='afe-v2'?0x300000:0x1e0000;
  if(!this.sequence||this.minimumSequence>=this.sequence||field(manifest.subarray(48,80))!=='waveshare-esp32s3-audio'||!['afe-v1','afe-v2','afe-v3','owner-v1'].includes(this.layout)||image.length<1024||image.length>slotBytes||image.length!==manifest.readUInt32BE(12)||image[0]!==0xe9||image.readUInt16LE(12)!==9||!createHash('sha256').update(image).digest().equals(manifest.subarray(16,48)))throw new Error('Firmware image does not match its signed manifest');
